...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        // [IDENTIFY]
        const ownerNumber = "5562996638900@c.us"
        ...
        const mess = {
            wait: '[Espera opoha] Em andamento⏳ aguarde um momento',
            error: {
	...
	// Filter Banned People
        ...
        case 'speed':
            case 'ping':
                ...
                const deviceinfo = `- Level de Bateria : ${device.battery}%\n  ├ Esta Carregando : ${charged}\n  └ 24 Horas Online : ${device.is24h}\n  ├ OS Versão : ${device.phone.os_version}\n  └ Numero de Copilação : ${device.phone.os_build_number}\n\n _*Jam :*_ ${moment(t * 1000).format('HH:mm:ss')}`
                aruga.sendText(from, `*Device Info*\n${deviceinfo}\n\nPenggunaan RAM: *${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB*\nCPU: *${os.cpus().length}*\n\nStatus :\n- *${loadedMsg}* Mensagens Carregadas\n- *${groups.length}* Chats de Grupos\n- *${chatIds.length - groups.length}* Chats Privados\n- *${chatIds.length}* Total de Chats\n\nSpeed: ${latensi.toFixed(4)} _Second_`)
                break
                case 'setpic':
                    if (!isOwnerB) return aruga.reply(from, `Este comando so pode ser usado pelo Toin!`, id)
                    if (isMedia) {
                        ...
                        await aruga.setProfilePic(imageBase64)
                        aruga.sendTextWithMentions(from, `Makasih @${sender.id.replace('@c.us','')} Foto de Perfil 😘`)
                    } else {
                        aruga.reply(from, `Formato errado! \N⚠️ Envie imagens com #setprofilepic`, id)
                    }
                    break
        case 'getpic':
        ...
        case 'toinmenu':
            await aruga.sendText(from, menuId.textToin(pushname))
            .then(() => ((isGroupMsg) && (isGroupAdmins)) ? aruga.sendText(from, `Menu NSFW em desinvolvimento: *${prefix}toinmenu*`) : null)    
            break
        case 'tnc':
            await aruga.sendText(from, menuId.textTnC())
            break
        case 'Ajuda':
            const bots = `Olá Nami, aqui é Toin que faz figurar, para descobrir o menu de comandos, digite * ${prefix} menu * , *${prefix}p*`
            await aruga.reply(from, bots , id)
            break
        case 'p':
        case 'menu':
            await aruga.sendText(from, menuId.textMenu(pushname))
            .then(() => ((isGroupMsg) && (isGroupAdmins)) ? aruga.sendText(from, `Menu dos fodaum ae os adm: *${prefix}menuadmin*`) : null)
            break
            case 'resetstiker':
                case 'resetsticker':{
                    if (!isGroupAdmins) return aruga.reply(from, 'Esto comando so pode ser usado por adms mame a rola deles pedindo o seu')  
                    if (!args.length >= 1) return aruga.reply(from, `Digite o Numero, *GUNAKAN AWALAN 55*\ncontoh: ${prefix}resetsticker 62852262236155 / #resetsticker @member`) 
                    const nomer = args[0]
                    ...
                    const cus = text + '@c.us'
                        ...
                        if (found !== false) {
                            ...
                            const result = 'Toin Sticker Spam Resetado'
                            console.log(stickerspam[found])
                            fs.writeFileSync('./lib/helper/stickerspam.json',JSON.stringify(stickerspam));
                            aruga.reply(from, result, from)
                            limitAdd(serial)
                        } else {
                                aruga.reply(from, `${monospace(`Não há número no banco de dados, mano`)}`, id)
                        }
                    }
                break
        case 'menuadmin':
        ...
        case 'ownerbot':
            ...
            .then(() => aruga.sendText(from, 'Naum faça muitas perguntas pare de me Stalkear'))
            break
            case 'bokep2':
                if (!isOwnerB) return aruga.reply(from, 'Fif (!if (Este recurso só pode ser usado pelo Owner Bot, por medo de abuso)', id)
			    rugaapi.bokep2()
			    .then(async (res) => {
            ...
            case 'wallpaper':
                ...
                break
                case 'loli (naum use isso seu merda)':
                aruga.reply(from, mess.wait, id);
            ...
            case 'Hentai':
                if (!isGroupMsg) return aruga.reply(from, 'Este recurso só pode ser usado em grupos', id)
                    ...
                    axios.get('https://www.amadoraslindas.com/fotos-de-hentai/').then(res => {
            ...
            case 'baka':
                ...
                case 'antilink':
                    if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro de grupos!', id)
                    if (!isGroupAdmins) return aruga.reply(from, 'Este comando so pode ser usado pelos adm mame um e garanta o seu', id)
                    if (!isBotGroupAdmins) return aruga.reply(from, 'Ó administrador, torne-me o administrador do grupo primeiro mama o adm logo :)', id)
                    if (args[0] == 'on') {
                        ...
                        var cek = antilink.includes(chatId);
                        if(!cek){
                            return aruga.reply(from, '*O detector de link anti-grupo * não está mais ativo neste grupo', id) //if number already exists on database
                        } else {
                            let nixx = antilink.indexOf(chatId)
                            antilink.splice(nixx, 1)
                            fs.writeFileSync('./lib/helper/antilink.json', JSON.stringify(antilink))
                            aruga.reply(from, '*[Anti Link de Grupo] * foi desativado\n', id)
                        }
                    } else {
        ...
        if (ar[0] === 'enable') {
            ...
            await aruga.reply(from, ind.wrongFormat(), id)
                    ...
                    case 'ava':
                    if (!isGroupMsg) return aruga.reply(from, 'Este recurso só pode ser usado em grupos', id)
                    if (!quotedMsg) return aruga.reply(from, 'Citar / responder mensagem para alguém que irá baixar a foto !!', id)
                    try {
                    ...
                    const caption = `*Detalhe do Membro* ✨ \n\n● *Nome :* ${nama}\n● *Bio :* ${bio.status}\n● *Link do Chat :* wa.me/${sender.id.replace('@c.us', '')}\n● *Cargo :* ${adm}\n● *Banido pelo bot do Toin:* ${ban ? '✔' : '❌'}\n● *Bloqueado pelo bot do Toin :* ${isblocked ? '✔' : '❌'}\n● *Converse com o bot do Toin :* ${isOnline}`
                ...
                case 'jadian':
                    ...
                    const sapa = `Cieee... @${aku.replace(/[@c.us]/g, '')} (💘) @${kamu.replace(/[@c.us]/g, '')} Acabou de Rolar Aqui \nPara o pj Por favor`
                    await aruga.sendTextWithMentions(from, sapa)
                    break     
                    case 'antisticker':
                    if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro de grupos!', id)
                    ...
                    if (args[0] == 'on') {
                        ...
                        if(cek){
                            return aruga.reply(from, '*Detector de Stickers anti-spam * já está ativo neste grupo', id) //if number already exists on database
                        } else {
                            antisticker.push(chatId)
                            fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(antisticker))
                            aruga.reply(from, '*[Anti Sticker SPAM] * foi ativado\nCada membro do grupo cujo adesivo de spam for maior que 7 será chutado pelo bot!', id)
                        }
                    } else if (args[0] == 'off') {
                        var cek = antilink.includes(chatId);
                        if(cek){
                            return aruga.reply(from, '* Detector de adesivos anti-spam * não está mais ativo neste grupo', id) //if number already exists on database
                        } else {
                            let nixx = antisticker.indexOf(chatId)
                            antisticker.splice(nixx, 1)
                            fs.writeFileSync('./lib/helper/antisticker.json', JSON.stringify(antisticker))
                            aruga.reply(from, '*[Anti Sticker SPAM] * foi desativado\n', id)
                        }
                    } else {
                        aruga.reply(from, `selecione ligar / desligar\n\n* [Anti Sticker SPAM] * \nCada membro do grupo que for spam adesivo será chutado por um bot!`, id)
                    }
                    break          
            case 'resend':
                if (!isGroupAdmins) return aruga.reply(from, 'Falha recurso so pode ser usado pelo Adm',id)
                if (quotedMsgObj) {
                    let encryptMedia
            ...
            case 'bkp': // MFARELS
                if (!isOwnerB) return aruga.reply(from, 'Este comando só pode ser usado pelo Owner Bot, por medo de abuso', id) // MFARELS
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `e aí vocês, eu sou o Toin que faz figurar. Para descobrir os comandos neste tipo de bot ${prefix}menu`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `Desculpe, o grupo neste bot está cheio \nMax Group está: ${groupLimit}`, id)
        ...
        case 'Nome':
                ...
                case 'read':
                    ...
                    try {
                        ...
                        aruga.sendTextWithMentions(from, `Basta ngeread .. Vamos agora\n${list}`)
                    } catch(err) {
                        console.log(err)
        ...
        case 'botstat': {
            ...
            const chatIds = await aruga.getAllChatIds()
            const groups = await aruga.getAllGroups()
            const groupsIn = groups.filter(x => x.groupMetadata.participants.map(x => [botNumber, '556299623471@c.us'].includes(x.id._serialized)).includes(true))
	...
	case 'stmg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                .then(() => {
                    console.log(`Aguarde processando sticker ${processTime(t, moment())} Segundos`)
                })
        } else if (!quotedMsg) return aruga.reply(from, `Formato incorreto, marque o Sticker que deseja usar como imagem!`, id)
        break
	...
	case 'brainly':
                ...
                case 'sgif':
                    if (isMedia) {
                        if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                            ...
                            aruga.reply(from, '[Aguarde] Processando seu Sticker Animado', id)
                            ...
                            aruga.reply(from, `[❗] Opoha *${prefix}stickergif* Maximo 10 segundos!`, id)
                        )
                    }
                    break
        case 'stikergiphy':
        case 'stickergiphy':
            ...
            if (isGiphy) {
                ...
                await aruga.reply(from, 'Desculpe, o adesivo de comando giphy só pode usar links de giphy. [Somente Giphy]', id)
            }
            break
        case 'meme':
    ...
    case 'linkgc':
            ...
            if (isGroupMsg) {
                ...
                aruga.sendLinkWithAutoPreview(from, inviteLink, `\nGrupo de links * ${name} * Use * ${prefix} revoke * para redefinir o grupo de links`)
            } else {
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Este comando só pode ser usado quando o bot se torna administrador', id)
                    if (isBotGroupAdmins) {
                        aruga
                            ...
                            .then((res) => {
                                aruga.reply(from, `Revogar Grupo de Links com sucesson *${prefix}link de grupo * para obter o link de convite de grupo mais recente`, id);
                            })
                            .catch((err) => {
                                console.log(`[ERR] ${err}`);
                            });
	...
	case 'dewabatch':
        ...
        case 'whatanime':
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                aruga.reply(from, 'Procurano sapoha', id)
	    ...
	    case 'add':
            ...
            if (!isBotGroupAdmins) return aruga.reply(from, 'Falha, se eu quiser usar este recurso, serei o administrador', id)
	        if (args.length !== 1) return aruga.reply(from, `Para usar ${prefix} add \nUsage: ${prefix} add <number> \nexample: ${prefix} add 5562xxx`, id)
                try {
        ...
        case 'kick':
            ...
            await aruga.sendTextWithMentions(from, `Done!, mengeluarkan ${mentionedJidList.map(x => `@${x.replace('@c.us', '')} a fim de se tornarem filhos adotivos`).join('\n')}`)
            for (let i = 0; i < mentionedJidList.length; i++) {
                if (groupAdmins.includes(mentionedJidList[i])) return await aruga.sendText(from, 'GOBLOK, como você pode chutar um administrador naum da ne opoha')
            ...
            case 'promote':
                ...
                await aruga.sendTextWithMentions(from, `Pronto, ciee, @${mentionedJidList[0].replace('@c.us', '')} O grau é aumentado xixi.`)
                break
            case 'demote':
                if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro de grupos!', id)
                if (!isGroupAdmins && !isOwnerB) return aruga.reply(from, 'Falha, este recurso funcionará se usado pelo administrador, mas os membros não precisam ser difíceis', id)
            ...
            case 'bye':
                if (!isGroupMsg) return aruga.reply(from, 'Desculpe, este comando só pode ser usado dentro de grupos!', id)
                if (!isGroupAdmins && !isOwnerB) return aruga.reply(from, 'Falha, este recurso funcionará se usado pelo administrador, mas os membros não precisam ser difíceis', id)
                aruga.sendText(from, 'Evil kelenen comigo... ( ⇀‸↼‶ )').then(() => aruga.leaveGroup(groupId))
                break
            case 'del':
                if (!isGroupAdmins && !isOwnerB) return aruga.reply(from, 'Falha, este recurso funcionará se usado pelo administrador, mas os membros não precisam ser difíceis', id)
        ...
        if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
            ...
            aruga.reply(from, 'Sucesso eu kikar todo mundo', id)
        break
        ...
        case 'ban':
            ...
            case 'google':
                ...
                case 'randomanimehentai':
                        if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                        aruga.sendText(from, mess.wait);
                        axios.get(`https://api.computerfreaker.cf/v1/anime`).then(res => {
                        aruga.sendImage(from, res.data.url, 'hentao.jpg', 'Eitar Gozei', id)
        })
                        break
                case 'randomanimetrap':
                      if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                      aruga.sendText(from, mess.wait);
                      axios.get(`https://api.computerfreaker.cf/v1/trap`).then(res => {
                      aruga.sendImage(from, res.data.url, 'hentao.jpg', id)
    })
                        break
            case 'Trap':
                    if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
                    const pptl = ("https://api.computerfreaker.cf/v1/trap")
                    let pep = pptl[Math.floor(Math.random() * pptl.length)]
                    aruga.sendFileFromUrl(from, pep, 'trap.jpg', 'nihh ngab', id)
                    break
            case 'groupinfo' :
            case 'gcinfo' :
